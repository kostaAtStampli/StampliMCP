...
public static class OperationTools
{
    ...
    public static async Task<object> GetOperation(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return operation is not null 
            ? operation.ToToolResult() 
            : new { error = $"Operation '{methodName}' not found" };
    }

    [McpServerTool(Name = "list_operations")]
    [Description("List all operations in a specific category or all categories")]
    public static async Task<object> ListOperations(
        KnowledgeService knowledge,
